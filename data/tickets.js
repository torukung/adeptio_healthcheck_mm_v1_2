 ...
 * renders them through dstamp() -> "D5 19:30". "now" = 2015 (= the dashboard's
...
{ key:"INC-1012", type:"incident", major:false, status:"done", priority:"high",
  ...
  desc:"D1 dawn. smsgw.DLR rate (worst carrier) dropped from a 98.6% baseline to roughly 73% while every other carrier stayed nominal, and otp.Challenge-to-verify success followed it down. Time-to-deliver p90 stretched past 45s.\n\nSingle-carrier, vendor-side. The bank's OTP service issued challenges normally throughout - the codes simply did not arrive. Customer-visible symptom was an OTP screen that times out.",
...
{ key:"INC-1023", type:"incident", major:false, status:"done", priority:"high",
  ...
  desc:"D3 morning. Two symptoms that look like two incidents: customers on one carrier saw slow or failing app traffic (telco.TCP connect p95 and TLS handshake p95 both stretched, packet loss up) while OTP delivery on the same carrier degraded.\n\nOne root cause. The carrier's transit and its SMSC path share infrastructure, so a single upstream fault surfaced on both legs of the journey. Splitting by carrier is what collapses it from two tickets into one.",
...
{ key:"INC-1025", type:"bug", major:false, status:"done", priority:"medium",
  ...
  desc:"The per-carrier DLR panel was averaging across carriers before comparing to the band, so a single carrier collapsing to 70% barely moved the number while two healthy carriers held the average up. The D1 dip was caught by the time-to-deliver probe rather than by the DLR band it should have tripped.\n\nFix: the monitor now evaluates the WORST carrier against the band, and the objective is labelled as such.",
  assignee:O.smsgw, reporter:O.otp.name, labels:["monitoring","sms","bug"],
  incKey:null, node:"smsgw", created:880, updated:1120,
  comments:[
    {who:O.smsgw.name, at:900,  text:"Reproduced against the D1 window - the aggregate never left green while one carrier sat at 73%."},
    {who:O.smsgw.name, at:1116, text:"Objective renamed to DLR rate (worst carrier) and the band re-evaluated per carrier. Replayed D1 and it trips correctly now."}
  ],
  history:[
...
{ key:"INC-1027", type:"incident", major:false, status:"done", priority:"high",
  ...
  desc:"D4 06:00 onward. Not an outage: a ramp. Storage service time on the core database estate drifted upward over a day and a half with no single step change, dragging core write latency and gateway route p95 along behind it.\n\nThis is the boiling-frog shape - every individual five-minute sample looks acceptable and the trend is unmistakable. It was raised at Low, escalated twice as the slope held, and it is the precondition that made the D5 failover fail.",
...
{ key:"INC-1029", type:"task", major:false, status:"inprog", priority:"low",
  ...
  desc:"The current runbook triages declines by symptom, which is why the D2 false-decline case took twenty minutes to name. Rewrite it around the fault-fingerprint matrix on the flow-instrumentation page: which objectives go red together, and what that combination rules out.\n\nDeliverable is one page per fingerprint with the first three checks and the question that separates it from its nearest neighbour.",
...
{ key:"INC-1034", type:"incident", major:false, status:"done", priority:"high",
  ...
  desc:"D6 afternoon. Two of the four load-balancer pool members dropped out of rotation, halving perimeter capacity. edge.Pool members up fell to 2, TLS termination p95 rose on the survivors, and the gateway saw the queueing behind it.\n\nNo customer-visible outage: the remaining pair carried the load at degraded latency. This is the case for watching pool membership as its own objective rather than inferring it from response time.",
...
{ key:"INC-1036", type:"incident", major:false, status:"done", priority:"medium",
  ...
  desc:"D6 night. The end-of-day batch ran long and pushed the biller advice file close to its cut-off: bbatch.Cut-off buffer fell from 90 minutes to 12, advice-file age rose, and the retry queue built.\n\nThe file made the window. Flagged as a known window rather than a fault - the batch has been trending longer as volume grows, and the fix is a capacity conversation, not an incident response.",
...
{ key:"INC-1039", type:"incident", major:false, status:"inprog", priority:"high",
  ...
  desc:"D7 morning. bhub.Adapter error and bhub.Biller timeout rose together across more than one biller, with inquiry p95 stretching past band. Partial, not total: most payments completed, a minority failed at the credit leg and a smaller number left a debit awaiting credit.\n\nAggregator-side. The bank's hub was healthy; the fault was upstream of the adapters. Still open pending the vendor's root-cause statement - the symptom cleared on its own, which is exactly why we are not closing it.",
...
{ key:"INC-1042", type:"incident", major:false, status:"done", priority:"high",
  ...
  desc:"D7 evening, the most recent incident on the board. A gateway route-config deployment pushed a rule ordering that sent Pay Bill traffic through an extra authorisation hop. gw.Route p95 roughly doubled, gw.5xx rate crossed band, and pay.Technical decline followed.\n\nSharp onset, clear correlation with the deploy, rolled back inside half an hour. The corrected config still needs to go out - that is a separate item so this one can close.",
...
{ key:"INC-1043", type:"task", major:false, status:"todo", priority:"high",
  ...
  desc:"The D7 evening deploy was rolled back, so the change it carried is still outstanding. Re-cut the route configuration with the rule ordering corrected - Pay Bill must not traverse the extra authorisation hop - and re-deploy under the normal release process with a route p95 watch on the bridge.\n\nBlocked on nothing; waiting for the next release window.",
