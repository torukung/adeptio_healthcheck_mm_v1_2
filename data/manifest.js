...
const INC = {
  ...
  A:[408,428,460],     // D2 midday — MAJOR silent false-declines from replica lag (the L3 signature)
  E:[660,684,720],     // D3 morning — MULTI-PATH: one carrier degrades customer ingress AND SMS delivery
  B:[936,1368,1500],   // D4 06:00 → D5 18:00 peak → D5 21:00 clear — storage creep on the core DB estate, decays after the D5 outage response (post-incident fix)
  F:[1380,1394,1428],  // D5 19:00 → D6 02:00 — BACK-END DOWN: core outage after failed failover; full path red
  G:[1602,1614,1638],  // D6 13:30–16:30 — edge/LB pool loss (2 of 4 members)
  I:[1700,1710,1726],  // D6 night — EOD batch overrun
  H:[1788,1812,1848],  // D7 05:00–10:00 — biller hub / aggregator brownout
  D:[1932,1944,1962]   // D7 17:00–19:30 — gateway deploy regression (sharp, recent)
};
...
const NODES=[
 {id:'client',name:'Customer · Mobile App',ip:'handset · release vX.Y',type:T.client,x:80,y:330,objs:[
 ...
 {id:'telco',name:'Telco Data · DNS/TLS',ip:'4G APN · resolver · transit',type:T.net,x:230,y:330,objs:[
 ...
 {id:'edge',name:'Edge — FW · WAF · LB',ip:'perimeter · public VIP',type:T.fw,x:380,y:330,objs:[
   AV({G:2.2},1.5),
 ...
 {id:'gw',name:'API Gateway',ip:'POST /v1/bills/pay',type:T.gw,x:535,y:330,objs:[
   AV({F:2.8},2),
 ...
 {id:'auth',name:'Auth & Session',ip:'JWT · session cache',type:T.app,x:515,y:205,objs:[
 ...
 {id:'otp',name:'OTP & Notify Svc',ip:'challenge · verify · expiry',type:T.app,x:640,y:190,objs:[
   A('Challenge→verify success','%',97.5,94,90,'lo',9,0.4,{C:9,F:7},{max:100}),
   AV(null,2),
   A('OTP issue p95','ms',300,800,2000,'hi',350,60,{C:350,F:900})]},
 {id:'smsgw',name:'Telco SMS / OTP GW',ip:'SMPP bind · DLR receipts',type:T.link,x:665,y:305,objs:[
   AV({C:2.5},2.5),
   A('DLR rate (worst carrier)','%',98.6,97,90,'lo',26,0.3,{C:26,E:9},{max:100}),
   A('Time-to-deliver p90','s',8,15,45,'hi',60,1.5,{C:60,E:25})]},
 {id:'pay',name:'Payment Orchestrator',ip:'saga · retries · idempotency',type:T.app,x:665,y:430,objs:[
 ...
 {id:'acct',name:'Accounts & Balance',ip:'reads · statements',type:T.app,x:850,y:590,objs:[
 ...
 {id:'dbr',name:'DB Replica (balance)',ip:'read replica · SAN',type:T.db,x:1010,y:590,objs:[
   AV(null,1.2),
 ...
 {id:'core',name:'Core Banking',ip:'posting · journal · GL',type:T.core,x:1070,y:470,objs:[
   AV({F:3.2},1.5),
 ...
 {id:'bhub',name:'Biller Hub · Aggregator',ip:'routing · adapters · N billers',type:T.switch,x:1360,y:390,objs:[
   AV({H:2.4},2),
 ...
 {id:'biller',name:'Biller · Online',ip:'YESC · MESC · ESE · telco top-up',type:T.bank,x:1550,y:350,objs:[
 ...
 {id:'bbatch',name:'Biller · Advice File',ip:'batch post · confirm cycle',type:T.stor,x:1640,y:550,objs:[
 ...
 {id:'mq',name:'Queue & History',ip:'events · statement sync',type:T.mq,x:1200,y:590,objs:[
   AV(null,1.5),
 ...
 {id:'recon',name:'Recon & Settlement',ip:'T+1 · 3-way match',type:T.recon,x:1360,y:590,objs:[
   AV(null,1.2),
...
window.ADEPTIO_DATA = { N, STEP_MIN, DAY, INC, INCMETA, NODES, LINKS, KPI:KPI_DEF, TABLE_DEFAULTS };
})();
